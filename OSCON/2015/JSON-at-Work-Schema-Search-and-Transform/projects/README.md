...
The solution is available at: FIXME.
...
* Add a Mocha/Chai spec to validate the JSON Document against the Schema:
  * Create a Node project as follows:
    * FIXME
  * Create a Mocha/Chai spec:
    * Borrow a [Spec](FIXME) that will read and parse a JSON file.
    * You'll need to read in both the JSON Dcoument and Schema.
    * Add the [`chai-json-schema`](http://chaijs.com/plugins/chai-json-schema) npm module to validate the JSON document against the Schema.
